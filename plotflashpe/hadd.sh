...
OUTPUT=out_plotflashpe_mcc8v7_bnb_overlay_p01.root
...
singularity exec ${CONTAINER} bash -c "source /usr/local/bin/thisroot.sh && cd /tmp/ && hadd ${OUTPUT} ${WORKDIR_IC}/output/*.root && cp ${OUTPUT} ${WORKDIR_IC}/"
